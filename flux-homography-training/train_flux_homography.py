...
class FluxNetworkTrainer(train_network.NetworkTrainer):
    ...
    def assert_extra_args(self, args, train_dataset_group):
        ...
        # default is False even if args.network_train_unet_only is False
    ...
    def load_target_model(self, args, weight_dtype, accelerator):
        ...
        ae = flux_utils.load_ae(
            args.ae, weight_dtype, "cpu", disable_mmap=args.disable_mmap_load_safetensors)
    ...
    def cache_text_encoder_outputs_if_needed(
        ...
        if args.cache_text_encoder_outputs:
            ...
            text_encoders[0].to(accelerator.device,
                                dtype=weight_dtype)  # always not fp8
    ...
    def get_noise_pred_and_target(
        ...
        packed_noisy_model_input = flux_utils.pack_latents(
            noisy_model_input)  # b, c, h*2, w*2 -> b, h*w, c*4
        packed_latent_height, packed_latent_width = noisy_model_input.shape[
            2] // 2, noisy_model_input.shape[3] // 2
        img_ids = flux_utils.prepare_img_ids(
            bsz, packed_latent_height, packed_latent_width).to(device=accelerator.device)

        # get guidance
        # ensure guidance_scale in args is float
        guidance_vec = torch.full((bsz,), float(
        ...
        if "custom_attributes" in batch:
            ...
            if len(diff_output_pr_indices) > 0:
                ...
                target[diff_output_pr_indices] = model_pred_prior.to(
                    target.dtype)
