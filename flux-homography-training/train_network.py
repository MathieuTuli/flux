...
class NetworkTrainer:
        ...
        for i, lr in enumerate(lrs):
            ...
            if (
                args.optimizer_type.lower().endswith(
                    "ProdigyPlusScheduleFree".lower()) and optimizer is not None
    ...
    def sample_images(self, accelerator, args, epoch, global_step, device, vae, tokenizers, text_encoder, unet):
        train_util.sample_images(
            accelerator, args, epoch, global_step, device, vae, tokenizers[0], text_encoder, unet)
    ...
    def get_noise_pred_and_target(
        ...
        if "custom_attributes" in batch:
            ...
            if len(diff_output_pr_indices) > 0:
                ...
                target[diff_output_pr_indices] = noise_pred_prior.to(
                    target.dtype)
    ...
    def post_process_loss(self, loss, args, timesteps, noise_scheduler):
        if args.min_snr_gamma:
            loss = apply_snr_weight(
                loss, timesteps, noise_scheduler, args.min_snr_gamma, args.v_parameterization)
    ...
    def train(self, args):
        ...
        if args.debug_dataset:
            # dasaset needs to know the strategies explicitly
        ...
        text_encoders = text_encoder if isinstance(
            text_encoder, list) else [text_encoder]
        ...
        text_encoder_outputs_caching_strategy = self.get_text_encoder_outputs_caching_strategy(
            args)
        ...
        self.post_process_network(
            args, accelerator, network, text_encoders, unet)
        ...
        if support_multiple_lrs:
            ...
            if args.text_encoder_lr is None or isinstance(args.text_encoder_lr, float) or isinstance(args.text_encoder_lr, int):
                ...
                text_encoder_lr = None if len(
                    args.text_encoder_lr) == 0 else args.text_encoder_lr[0]
        try:
            if support_multiple_lrs:
        ...
        if args.deepspeed:
            ...
            ds_model = deepspeed_utils.prepare_deepspeed_model(
                ...
                text_encoder2=(text_encoders[1] if flags[1] else None) if len(
                    text_encoders) > 1 else None,
                network=network,
            )
            ...
            if train_unet:
                ...
                unet = self.prepare_unet_with_accelerator(
                    args, accelerator, unet)  # accelerator does some magic here
            else:
        ...
        if args.pretrained_model_name_or_path is not None:
            ...
            if os.path.exists(sd_model_name):
                metadata["ss_sd_model_hash"] = train_util.model_hash(
                    sd_model_name)
        ...
        if args.vae is not None:
            ...
            if os.path.exists(vae_name):
                ...
                metadata["ss_new_vae_hash"] = train_util.calculate_sha256(
                    vae_name)
        ...
        if hasattr(accelerator.unwrap_model(network), "on_step_start"):
            on_step_start_for_network = accelerator.unwrap_model(
        ...
        # For --sample_at_first
        optimizer_eval_fn()
        self.sample_images(accelerator, args, 0, global_step,
                           accelerator.device, vae, tokenizers, text_encoder, unet)
        optimizer_train_fn()
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            for step, batch in enumerate(skipped_dataloader or train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    self.on_step_start(
                        args, accelerator, network, text_encoders, unet, batch, weight_dtype)

                    if "latents" in batch and batch["latents"] is not None:
                        latents = batch["latents"].to(
                            accelerator.device).to(dtype=weight_dtype)
                    else:
                        with torch.no_grad():
                            # latentに変換
                    ...
                    if len(text_encoder_conds) == 0 or text_encoder_conds[0] is None or train_text_encoder:
                        ...
                        with torch.set_grad_enabled(train_text_encoder), accelerator.autocast():
                            ...
                            if args.weighted_captions:
                                ...
                                input_ids = [ids.to(accelerator.device)
                ...
                if args.scale_weight_norms:
                    ...
                    max_mean_logs = {"Keys Scaled": keys_scaled,
                ...
                if accelerator.sync_gradients:
                    ...
                    if args.save_every_n_steps is not None and global_step % args.save_every_n_steps == 0:
                        ...
                        if accelerator.is_main_process:
                            ...
                            save_model(ckpt_name, accelerator.unwrap_model(
                                network), global_step, epoch)
        ...
        if is_main_process:
            ...
            save_model(ckpt_name, network, global_step,
                       num_train_epochs, force_sync_upload=True)
